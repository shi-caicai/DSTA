...
class GraphLLM(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(args.llm_model_path, trust_remote_code = True, use_fast=False, revision=kwargs["revision"])

        if args.llm_model_name!='glm4-9b':
        ...
        if args.llm_frozen == 'True':
            ...
            # if args.llm_model_name=='llama2-7b' or args.llm_model_name=='llama2-13b':
            #     lora_target_modules = [
            #         "q_proj",
            #         "v_proj",
            #     ]
            ...
            if args.llm_model_name=='baicuan-7b':
        ...
        if args.llm_model_name == 'llama2-13b':
            self.projector = nn.Sequential(
                nn.Linear(args.gnn_hidden_dim, 2560),
                nn.Sigmoid(),
                nn.Linear(2560, 5120),
            ).to(self.model.device)
        else:
        ...
        if args.llm_frozen=='True' and args.llm_model_name=='glm4-9b':
            self.word_embedding = self.model.transformer.get_input_embeddings()
        else:
    ...
    def encode_graphs(self, samples):
        ...
        # 获取唯一元素
        unique_values = torch.unique(graphs.batch)
        graphs = graphs.to(self.model.device)

        # 只有一个节点或者无节点
        if graphs.x.shape[0]==1 or graphs.x.shape[0]==0:
        ...
        return g_embeds,unique_values

    def forward(self, samples):
        # encode questions and labels
        ...
        # encode graphs
        graph_embeds, has_embed_index = self.encode_graphs(samples)
        if graph_embeds != None:
        ...
        for i in range(batch_size):
            ...
            if graph_embeds!=None and i in has_embed_index:
